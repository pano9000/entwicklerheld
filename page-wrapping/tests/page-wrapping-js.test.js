import { buildPages } from "../page-wrapping-js";
...
test('test_1_1', () => {
    ...
    if (!actual || !actual.pages) {
        expect(false, `Your function should return an object {'pages': pages, 'hasToc': ${hasToc}}`).toBe(true);
    }

    visData = [1,1, html1];

    expect(actual.pages.length, `Expected length of the pages array is 3, but was ${actual.pages.length}`).toBe(3);
    expect(actual.hasToc, `Expected parameter hasToc should be ${hasToc}, but was ${actual.hasToc}`).toBe(hasToc);
});
...
test('test_2_1', () => {
    ...
    expect(actual.pages.length, `Expected length of the pages array is 9, but was ${actual.pages.length}`).toBe(9);
    expect(actual.hasToc, `Expected parameter hasToc should be ${hasToc}, but was ${actual.hasToc}`).toBe(hasToc);

    displayNode.innerHTML = actual.pages[0];
    const tocNode = document.getElementById('paged-toc-node');
    if (!tocNode) expect(false, `The first page should be the table of contents containing a node with id 'paged-toc-node', but was null.`).toBe(true);

    let classToken = 'paged-toc-h1';
    let headings = document.getElementsByClassName(classToken);
    let expectedHeadingsCount = 1;
    if (!headings || headings.length === 0) expect(false, `The first page should be the table of contents with all headings of the text having class '${classToken}', but was null.`).toBe(true);
    expect(headings.length, `There should be ${expectedHeadingsCount} h1 heading with class '${classToken}' in the ToC, but was ${headings.length}`).toBe(expectedHeadingsCount);
    let headingText = headings[0].textContent;
    expect(headingText.toLowerCase(), `There should be ${expectedHeadingsCount} h1 heading with class '${classToken}' and Text 'Table of Contents' in the ToC, but was '${headingText}'`).toBe('table of contents');

    classToken = 'paged-toc';
    headings = document.getElementsByClassName(classToken);
    expectedHeadingsCount = 10;

    if (!headings || headings.length === 0) expect(true, `The first page should be the table of contents with all headings of the text having class '${classToken}', but was null.`).toBe(true);
    expect(headings.length, `There should be ${expectedHeadingsCount} h2 headings with class '${classToken}' in the ToC, but was ${headings.length}`).toBe(expectedHeadingsCount);
});
...
test('test_2_2', () => {
    ...
    expect(actual.pages.length, `Expected length of the pages array is 8, but was ${actual.pages.length}`).toBe(8);
    expect(actual.hasToc, `Expected parameter hasToc should be ${hasToc}, but was ${actual.hasToc}`).toBe(hasToc);
...
function setVisualization(scenarioId, sentenceId, textData) {
    let code = fs.readFileSync("page-wrapping-js/page-wrapping-js.js", 'utf8');
    code = code.replace(/export /g, '');

    let iframe_content = fs.readFileSync("page-wrapping-js/visualization/iframe-template.html", 'utf8');
    ...
    let visualizationContent = fs.readFileSync("page-wrapping-js/visualization/visualization-template.html", 'utf8');
